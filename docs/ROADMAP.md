...
1. Continue incremental `aion-api` route extraction only where a remaining route group or shared surface still has enough cohesion to justify a dedicated module after the HTTP-ingestion split, with connector admin, TTN route surfaces, and worker management still planned separately.
...
- `aion-api` modularization is intentionally incremental; Milestones 61 through 75 establish safe extraction patterns for later route-level and shared-surface splits.
